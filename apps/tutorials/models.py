...
class Tutorials(Model):
    ...
    authors = ManyToMany('user', verbose_name='作者', nullable=False, required=True, collection_name='authors_tutorials')
...
class Chapters(Model):
    ...
    tutorial = Reference('tutorials', verbose_name='所属教程')
    ...
    format = Field(CHAR, max_length=1, verbose_name='格式', choices=get_var('TUTORIALS/format'))
    ...
    @classmethod
    def OnInit(cls):
        Index('chp_indx', cls.c.tutorial, cls.c.parent, cls.c.order)

class Chapters_Comments(Model):
    __verbose_name__ = '评论'
    
    chapter = Reference('chapters', verbose_name='所属章节')
    ...
    @classmethod
    def OnInit(cls):
        Index('chpcmt_indx', cls.c.chapter, cls.c.parent, cls.c.anchor)
